...
def config_template():
    ...
    if "package_name" in config_diff:
        ...
        replace_line_containing_string(
            "pyproject.toml", f'name = "{current_name}"', f'name = "{new_name}"'
        )
        replace_line_containing_string(
            "pyproject.toml", f'name = "{current_name}"', f'name = "{new_name}"'
        )
    ...
    if "font" in config_diff:
        ...
        if "font-family" in config_diff["font"]:
            ...
            replace_line_containing_string(
                f"{new_name}/plotnine/plotnine_extra.py",
                f'fontfamily="{font_family_prev}"',
                f'fontfamily="{font_family_prev}",',
                keep_leading_whitespace=True,
    ...
    if "plotnine" in config_diff:
        if "plotnine-theme" in config_diff["plotnine"]:
            for item in config_diff["plotnine"]['plotine-theme']:
                variable = item.upper().replace("-", "_")
                value = config["plotnine"]['plotnine-theme'][item]
                if not isinstance(value, (int, float)):
                    value = f'"{value}"'

                # it is either uppercase at the start of the script or lowercase in the def __init__ section.
                replace_line_containing_string(
                    f"{new_name}/plotnine/plotnine_theme.py",
                    f"{variable}",
                    f"{variable}={value}",
                ...
                replace_line_containing_string(
                    f"{new_name}/plotnine/plotnine_theme.py",
                    f"{variable.lower()}=",
                    f"{variable.lower()}={value},",
                    keep_leading_whitespace=True,
                )

        if "plotnine-extra" in config_diff["plotnine"]:
            for item in config["plotnine"]['plotnine-extra']:
                variable = item.lower().replace("-", "_")
                value = config["plotnine"]['plotnine-extra'][item]
                if not isinstance(value, (int, float)):
                    value = f"'{value}'"
                replace_line_containing_string(
                    f"{new_name}/plotnine/plotnine_extra.py",
...
def replace_line_containing_string(
    file_path: str,
    search_string: str,
    replacement: str,
    start_of_line_only: bool = False,
    ...
    for i, line in enumerate(lines):
        if start_of_line_only:
            if line.startswith(search_string):
                lines[i] = replacement + "\n"
                found = True
                if replace_all_matches == False:
                    break
        else:
            if search_string in line:
                if keep_leading_whitespace:
                    lines[i] = leading_whitespace(line) + replacement + "\n"
                else:
                    lines[i] = replacement + "\n"
                found = True
                if replace_all_matches == False:
